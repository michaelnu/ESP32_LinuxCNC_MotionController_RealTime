...
void espNowOnDataRecvClientMode(const uint8_t *mac, const uint8_t *incomingData, int len)
{
    ...
    { 
        // if rx example data struct
        memcpy(&myData, incomingData, sizeof(myData));
    }
    else if (len == sizeof(fb))
    { 
        // if rx feedback packet struct. Print it for testing
...
void loop_Core0_EspNowSenderClientTask(void *parameter)
{
    ...
    while (runLoops)
    {
        ...
        { 
            // Register peer. Send the local wifi mac to the remote motion controller
        ...
        { 
            // esp-now peer is configured - send test data struct every 5s. Check if last rx packet >3s then mark as stale connection and retry adding peer every 5s
            if (!configEspNowEnabled)
            {
